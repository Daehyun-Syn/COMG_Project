...
<head>
    ...
    <script>
        ...
        function file_upload(input){
            ...
            if (!fileCheck) {
                console.log("파일 없으므로 과제 내용만 생성 시작");
                let textarea = CKEDITOR.instances.board.getData();
                var assignment_name = document.getElementById("assignment_name").value;
                var assignment_deadline = document.getElementById("assignment_deadline").value;
                let data = {"board" : textarea, "group_seq" : '<%=group_seq%>', "assignment_name" : assignment_name, "assignment_deadline" : assignment_deadline };
                console.log(CKEDITOR.instances.board.getData());
                console.log(data);
                $.ajax({
                    url: '/COMG/CreateAssignmentLogic',
                    type: "GET",
                    ...
                    success: function(data){
                        if(data == 1){
                            ...
                            swal('과제 생성 성공!', "과제가 생성되었습니다", 'success')
                                .then(function(){
                                    location.href="/COMG/Group?GroupSEQ=<%=group_seq%>";
                                });
                        }else {
                            console.log("과제 생성 실패!");
                            swal('과제 생성 실패!', "오류로 인해 과제 생성이 실패하였습니다.", 'warning');
                        }
                    },
                    error: function (){
                        console.log("아작스 에러");
                        swal('과제 생성 실패!', "관리자에게 문의해 주세요.", 'warning');
                    }
                });

            }else {
                console.log("파일 존재하여 파일 업로드+과제 생성 시작");
                let textarea = CKEDITOR.instances.board.getData();
                console.log(textarea);
                var form = $('#form')[0];
                var formData = new FormData(form);
                assignment_name = document.getElementById("assignment_name").value;
                assignment_deadline = document.getElementById("assignment_deadline").value;
                formData.append("from_table", "CREATE_ASSIGNMENT")
                formData.append("COMG_assignment_Contents", CKEDITOR.instances.board.getData())
                ...
                $.ajax({
                    ...
                    error: function (){
                        console.log("아작스 에러");
                        swal('과제 생성 실패!', "파일 용량이 10mb를 초과합니다.", 'warning');
                    }
                });
            }
        }
    ...
    <div class="adjoined-bottom">
        <div class="grid-container">
            ...
            <div class="grid-width-100">
                    ...
                    <div style="float: top">
                <span class="form-label">과목명을 입력하세요.</span> <span  class="form-label" style="float: right; padding-right: 25.3%">과제 마감일을 입력하세요.</span>
                    </div>
                <input type="text" class="input-group-text w-55" name="assignment_name" id="assignment_name" style="float: left" required="required">

                <input type="date" class="input-group-text w-40" name="assignment_deadline" id="assignment_deadline" style="float: right" required="required">
                <div>&nbsp;&nbsp;&nbsp;&nbsp;</div>
                ...
                <div class="text-center">
                    ...
                    <button type="button" class="btn bg-gradient-info w-90 my-4 mb-2" onclick="file_upload(this);">생성하기</button>
                </div>
            </form>
        </div>
    </div>
