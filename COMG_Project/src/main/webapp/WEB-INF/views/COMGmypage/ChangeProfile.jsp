...
<head>
    ...
    <script>
        ...
        function file_upload(input){
            ...
            if (!fileCheck) {
                console.log("파일 없으므로 alert띄워줌");
                swal('프로필 사진 변경 실패', "이미지 파일을 등록해주세요!", 'warning');
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if(data == 1){
                            ...
                            swal('프로필 변경 성공!', "프로필 사진 변경이 성공하였습니다.", 'success')
                                .then(function(){
                                    location.href="${pageContext.request.contextPath}/COMG/Mypage";
                                });

                        }else {
                            console.log("그룹 생성 실패!");
                            swal('프로필 변경 실패!', "오류로 인해 프로필 사진 변경이 실패하였습니다.", 'warning');
                        }
                    },
                    error: function (){
                        console.log("아작스 에러");
                        swal('프로필 변경 실패!', "파일 용량이 10mb를 초과합니다.", 'warning');
                    }
                });
            }
        }
            ...
            <div class="col-6 mx-auto">
                <div class="card mt-4">
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-dark shadow-danger border-radius-lg pt-4 pb-3">
                            <h4 class="text-white font-weight-bold text-capitalize ps-3">프로필 사진 변경</h4>
                            <p class="mb-0 text-white font-weight-bold ps-3">변경할 사진을 업로드해 주세요.
                    ...
                    <form id="form" class="card-body" method="post">
                        ...
                        <div class="mb-3">
                            ...
                            <br>
                            <div>
                                <label for="uploadImgUser" class="form-label">변경할 사진을 넣어주세요.</label>
                                <input class="form-control" type="file" id="uploadImgUser" name="uploadImgUser" onchange="prev_img(this)"/>
                            </div>
                                <input type="hidden" value="USER_INFO" name="from_table" id="from_table">
                        </div>


                        <div class="text-center">
                            <button type="button" class="btn bg-gradient-dark w-100 my-4 mb-2" onclick="file_upload(this);">변경</button>
                        </div>
                    </form>
                    </div>
                </div>
