...
<%
    ...
    CBoardDTO oDTO = (CBoardDTO)request.getAttribute("oDTO");
    String board_seq = CmmUtil.nvl(oDTO.getBoard_seq());
%>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="utf-8">
    <title>게시글 수정 | COMG</title>
    <!--     Fonts and icons     -->
    ...
    <script>
        ...
        function file_upload(input){
            ...
            if (!fileCheck) {
                console.log("파일 없으므로 게시글만 수정 시작");
                let textarea = CKEDITOR.instances.board.getData();
                let data = {"board" : textarea, "board_seq" : <%=board_seq%>};
                console.log(CKEDITOR.instances.board.getData());
                console.log(data);
                $.ajax({
                    url: '/COMG/BoardUpdateLogic',
                    type: "GET",
                    ...
                    success: function(data){
                        if(data == 1){
                            ...
                            swal('게시글 수정 성공!', "게시글이 수정되었습니다", 'success')
                                .then(function(){
                                    location.href="/COMG/Group?GroupSEQ=<%=oDTO.getFk_group_seq()%>";
                                });
                        }else {
                            console.log("게시글 수정 실패!");
                            swal('게시글 수정 실패!', "오류로 인해 게시글 수정이 실패하였습니다.", 'warning');
                        }
                    },
                    error: function (){
                        console.log("아작스 에러");
                        swal('게시글 수정 실패!', "파일 용량이 10mb를 초과합니다.", 'warning');
                    }
                });

            }else {
                console.log("파일 존재하여 파일 +게시판 업데이트 시작");
                let textarea = CKEDITOR.instances.board.getData();
                console.log(textarea);
                var form = $('#form')[0];
                var formData = new FormData(form);
                formData.append("from_table", "BOARD_UPDATE")
    ...
    <div class="adjoined-bottom">
        <div class="grid-container">
            ...
            <div class="grid-width-100">
                ...
                <textarea id="board" name="board"><%=oDTO.getBoard_contents()%></textarea>
