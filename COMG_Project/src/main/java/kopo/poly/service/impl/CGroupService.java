...
public class CGroupService implements ICGroupService {
    ...
    @Override
    public CGroupDTO getGroupUserCount(CGroupDTO gDTO) throws Exception {
        log.info("CGroupService : getGroupUserCount 시작!");

        if (gDTO == null){
            log.info("CGroupService : sDTO가 null이라 강제로 메모리에 올림!");
            gDTO = new CGroupDTO();
        }

        CGroupDTO oDTO = null;
        oDTO = new CGroupDTO();
        oDTO = cgroupMapper.getGroupUserCount(gDTO);
        if (oDTO.getGroup_user_count() == 0){
            log.info("CGroupService : 아무도 그룹에 가입하지 않음");
            oDTO = new CGroupDTO();
        }else {
            log.info("CGroupService : 그룹 가입유저 있음 : "+oDTO.getGroup_user_count()+"명");
        }

        log.info("CGroupService : getGroupUserCount 끝!");
        return oDTO;
    }

    @Override
    public CGroupDTO getGroupSeq(CGroupDTO nDTO) throws Exception {
        log.info("CGroupService : getGroupSeq 시작!");

        if (nDTO == null){
            log.info("CGroupService : nDTO가 null이라 강제로 메모리에 올림!");
            nDTO = new CGroupDTO();
        }

        CGroupDTO oDTO = null;
        oDTO = new CGroupDTO();
        oDTO = cgroupMapper.getGroupSeq(nDTO);

        if (oDTO == null){
            log.info("CGroupService : 그룹 SEQ값 가져오기 실패");
            oDTO = new CGroupDTO();
        }else {
            log.info("CGroupService : 그룹 SEQ값 가져오기 성공 "+oDTO.getGroup_seq());
        }

        log.info("CGroupService : getGroupSeq 끝!");
        return oDTO;
    }

    @Transactional
    @Override
    public int InsertGroupUserAdminInfo(CGroupDTO nDTO) throws Exception {
        log.info("CGroupService : InsertGroupUserAdminInfo(그룹 가입 로직) 시작 !");
        // 그룹가입 성공: 1,
        int res = 0;
        ...
        int success = cgroupMapper.InsertGroupUserAdminInfo(nDTO);

        //db에 데이터가 등록되었다면,
        if(success > 0) {
            res=1;
            log.info("CGroupService : InsertGroupUserAdminInfo, 그룹 어드민 등록 성공");
        }else {
            log.info("CGroupService : InsertGroupUserAdminInfo, 그룹 어드민 등록 실패");
        }
    ...
    @Override
    public CGroupDTO selectGroupUserSeq(CGroupDTO cDTO) throws Exception {
        log.info(this.getClass().getName()+"그룹 유저 시퀀스 가져오기 시작!");

        if(cDTO==null){
            cDTO = new CGroupDTO();
        }

        CGroupDTO nDTO = cgroupMapper.selectGroupUserSeq(cDTO);
        if(nDTO == null){
            nDTO = new CGroupDTO();
        }
        log.info(this.getClass().getName()+"그룹 유저 시퀀스 가져오기 종료!");
        return nDTO;
    }

    @Override
    public int UpdateGroupMsg(CGroupDTO bDTO) throws Exception {
        log.info("CGroupService : UpdateGroupMsg 시작 !");
        int res = 0;
        ...
        int success = cgroupMapper.UpdateGroupMsg(bDTO);
        ...
        log.info("CGroupService : UpdateGroupMsg 끝 !");
        return res;
    }
}
