...
<head>
    ...
    <script>
        ...
        function file_upload(input){
            ...
            if (!fileCheck) {
                console.log("파일 없으므로 사진제외한 그룹정보 업데이트 시작");
                var form = $('#form')[0];
                let data = new FormData(form);
                console.log(data);
                $.ajax({
                    url: '/COMG/EditGroupLogic',
                    type: "POST",
                    ...
                    success: function(data){
                        if(data == 1){
                            ...
                            swal('그룹 정보 수정 성공!', "그룹 정보 수정이 성공하였습니다.", 'success')
                                .then(function(){
                                    location.href="${pageContext.request.contextPath}/COMG/Group?GroupSEQ=<%=rDTO.getGroup_seq()%>";
                                });

                        }else {
                            console.log("그룹 프로필 변경 실패!");
                            swal('그룹 정보 수정 실패!', "오류로 인해 그룹 정보 수정이 실패하였습니다.", 'warning');
                        }
                    },
                    error: function (){
                        console.log("아작스 에러");
                        swal('그룹 정보 수정 실패!', "관리자에게 문의해주세요.", 'warning');
                    }
                });

            }else {
                console.log("파일 존재하여 프로필 사진 업로드 시작");
                var form = $('#form')[0];
                let data = new FormData(form);
                console.log(data);
                $.ajax({
                ...
                <div class="card mt-4">
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-success shadow-danger border-radius-lg pt-4 pb-3">
                            <h4 class="text-white font-weight-bold text-capitalize ps-3">그룹 정보 수정</h4>
                            <p class="mb-0 text-white font-weight-bold ps-3">수정할 그룹의 정보를 입력해 주세요.
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <div>
                                ...
                                <input type="hidden" name="from_table" value="UPDATE_GROUP">
                                <input class="form-control" type="file" id="uploadImgUser" name="uploadImgUser" onchange="prev_img(this)" />
                            </div>
                        ...
                        <div class="mb-3">
                            <div class="input-group">
                                ...
                                <textarea class="form-control" aria-label="With textarea" placeholder="그룹의 소개를 입력해주세요." name="group_introduce"><%=rDTO.getGroup_introduce()%></textarea>
                            </div>
                        </div>
                        <div class="text-center">
                            <button type="button" class="btn bg-gradient-success w-100 my-4 mb-2" onclick="file_upload(this);">그룹 수정</button>
                        </div>
                    </div>
                    </form>
                </div>
